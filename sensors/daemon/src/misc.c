 ...
 *
 * @file         misc.c
 * @date         "Thu May 30 17:54:37 2013 +0800"
 * @commit       "8c228b5"
 *
 * @brief
 *
...
#define SENSORD_VER_FMT "daemon_ver: %X(H) desc: %s info: %s\n"
#define ALGO_VER_FMT    "algo_ver: %c %X(H) desc: %s\n"

void dump_ver()
{

	PINFO(SENSORD_VER_FMT,
			SENSORD_VERSION, SENSORD_RLS_TIME, SENSORD_RLS_INFO);

	// hill, modify it to get version from lib core
	ts_version version;
    #ifdef CFG_BSC_PROJECT
	(void)bsc_getVersion(&version);
	PINFO("BSC project\n");
    #else
	(void)bsx_getVersion(&version);
	PINFO("BSX project\n");
    #endif
	PINFO("algorithm version: %d.%d.%d\n", version.major, version.minor, version.bugFix);
}
