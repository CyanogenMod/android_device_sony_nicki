...
#if !(defined HW_A_USE_INPUT_EVENT_CACHE) && !(defined HW_A_USE_INPUT_EVENT)
	if (-1 != g_fd_update_a) {
		lseek(g_fd_update_a, 0, SEEK_SET);
		tmp = write(g_fd_update_a, "1", 1);
	}
#endif
...
static int hw_acc_read_xyzdata(void *data)
{
	...
	val->x = val->x >> (16 - HW_INFO_DATA_BITS_A);
	val->y = val->y >> (16 - HW_INFO_DATA_BITS_A);
	val->z = val->z >> (16 - HW_INFO_DATA_BITS_A);
#endif
	return err;
}
...
#if ((defined HW_A_USE_INPUT_EVENT) || (defined HW_A_USE_INPUT_EVENT_CACHE))
...
static int hw_init_a(struct sensor_hw *hw)
{
	...
	g_input_dev_num_a = sysfs_get_input_dev_num(DEV_NAME_A);
	...
	g_fd_update_a = sysfs_open_input_dev_node(g_input_dev_num_a,
			SYSFS_NODE_NAME_BMA_UPDATE, O_WRONLY);


	PINFO("g_fd_value_a: %d g_fd_update_a: %d", g_fd_value_a, g_fd_update_a);
	...
	hw_a->data_bits = HW_INFO_DATA_BITS_A;
